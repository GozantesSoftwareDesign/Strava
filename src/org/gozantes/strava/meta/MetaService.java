...
public class MetaService extends Thread {
	...
	public void run() {
		...
		try {
			...
			Logger.getLogger().info(" Meta Service Received data from '" + tcpSocket.getInetAddress().getHostAddress() + ":" + tcpSocket.getPort() + "' -> '" + data + "'");					
			this.SaveUser(data);
		...
		if (msg != null && !msg.trim().isEmpty()) {
			try {
				...
				if (type.equals(CredType.Meta)) {
					...
					user = new User(new UserCredentials(type, id, passwd), new UserData(name, birth, weight, height, new Pair<Integer, Integer>(MaximunHeartRate, RestingHeartRate)));
