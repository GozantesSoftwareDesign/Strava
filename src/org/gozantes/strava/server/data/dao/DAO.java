...
public abstract class DAO <T> implements IDAO <T> {
    ...
    public void delete (T object) {
        ...
        try {
            ...
            Logger.getLogger ()
                    .warning (String.format ("Could not delete object %s: %s", object.toString (), e.getMessage ()));
        }
        finally {
    ...
    public void store (T object) {
        ...
        try {
            ...
            Logger.getLogger ()
                    .warning (String.format ("Could not store object %s: %s", object.toString (), e.getMessage ()));
        }
        finally {
            if (tx != null && tx.isActive ())
                tx.rollback ();
    ...
    public T find (String param) {
        ...
        try {
            ...
            Logger.getLogger ().info (ret == null
                    ? String.format ("%s not found.", this.instance ().objClass.getName ())
                    : String.format ("%s found.", ret.toString ()));
        }
        catch (Exception e) {
            Logger.getLogger ().warning (
                    String.format ("There was an error trying to find %s %s: %s", this.instance ().objClass.getName (),
                            this.instance ().pkGetter.getName ().startsWith ("get")
                                    ? ("by " + this.instance ().pkGetter.getName ().substring ("get".length ())
                                    .toLowerCase ())
                                    : ("using method" + this.instance ().pkGetter.getName ()), e.getMessage ()));
        }
        finally {
            if (tx != null && tx.isActive ())
                tx.rollback ();
