...
public record ServerParams(String name, Inet4Address ip, int port) {
    ...
    public ServerParams(String name, String ip) throws UnknownHostException {
        this(name, ip == null ? ServerParams.defaults.ip : (Inet4Address) Inet4Address.getByName(ip), ServerParams.defaults.port);
    }
    ...
    public ServerParams(String name, String ip, String port) throws UnknownHostException, NumberFormatException {
        this(name, ip == null ? ServerParams.defaults.ip : (Inet4Address) Inet4Address.getByName(ip), port == null ? ServerParams.defaults.port : Integer.parseInt(port));
    }
    ...
    public ServerParams(String name, Inet4Address ip, int port) {
        ...
        if (this.port < 0 || this.port > (2 << 15) - 1)
            throw new RangeException(RangeException.BAD_BOUNDARYPOINTS_ERR, "Invalid port (" + Integer.toString(this.port) + ")");
    }
